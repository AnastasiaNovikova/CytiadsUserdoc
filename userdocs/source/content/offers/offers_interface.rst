...
.. tip:: На самом деле, подключать оффер не нужно, потому что большая часть офферов уже подключена. Только в том случае если :ref:`статус аккаунт не позволяет<account-status-label>` работать с оффером, то оффер надо подключить через техподдержку. 
